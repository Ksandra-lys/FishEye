import { displayFooterTotalLikes } from "../widgets/displayFooterTotalLikes.mjs"
import type { Media } from "./type.mjs"
export function likeAndDislike(data : Media []) : void{
    const likeButtons = document.querySelectorAll(".like_btn") as NodeListOf<HTMLElement>
    const arrayOfLikedPictures = new Set ()
    likeButtons.forEach((btn)=>{
        btn.addEventListener("click", function(){
        const card = this.closest(".pictures") as HTMLElement
        let mediaId = Number(card.dataset["id"])
        const media = data.find((item)=> item.id === mediaId)

        if(!media) return;

        if(arrayOfLikedPictures.has(mediaId)){
            media.likes -= 1
            arrayOfLikedPictures.delete(mediaId)
        } else{
            media.likes += 1
            arrayOfLikedPictures.add(mediaId)
        }
        (card.querySelector(".like_count") as HTMLElement).textContent = String(media.likes)
        displayFooterTotalLikes(data)
    })
    })
}